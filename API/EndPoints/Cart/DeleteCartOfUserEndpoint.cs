...
namespace API.EndPoints.Cart
{
    ...
    {
        public DeleteItemCartCustomerHandler _handler { get; set; } = null!;
        ...
        public override async Task HandleAsync(ReqDeleteCartDto req, CancellationToken ct)
        {
            ...
            var result = await _handler.HandleAsync(new DeleteItemCartCustomerCommand(userId, req.productId));
            if (result.IsSuccess)
            {
